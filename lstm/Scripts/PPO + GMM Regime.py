# === PPO + GMM Regime Portfolio Trainer ===
...
gmm_regimes = pd.read_csv("gmm_regime_preds.csv").values.argmax(axis=1)
...
# === Setup and Train ===
env = DummyVecEnv([lambda: GMMEnv(returns, gmm_regimes)])
model = PPO("MlpPolicy", env, verbose=1)
model.learn(total_timesteps=100_000)
model.save("ppo_gmm_regime")
...
# === Plot NAV ===
plt.plot(navs)
plt.title("PPO + GMM Regime NAV Curve")
plt.xlabel("Step")
plt.ylabel("NAV")
plt.grid(True)
plt.savefig("ppo_gmm_nav_curve.png")
plt.show()
